...
# Function to detect objects in a frame with GPU acceleration
def detect_objects(frame, net, classes):
    height, width, _ = frame.shape
    blob = cv2.dnn.blobFromImage(frame, 1/255.0, (416, 416), swapRB=True, crop=False)
    
    # Use GPU acceleration with OpenCV for forward pass
...
def main():
    ...
    while True:
        ...
        # Measure FPS without GPU acceleration
        start_time = time.time()

        # Detect objects in the frame (without GPU acceleration)
        detect_objects(frame, net, classes)

        end_time = time.time()
        fps_no_gpu = 1 / (end_time - start_time)

        # Display FPS (without GPU acceleration)
        cv2.putText(frame, f'FPS (No GPU): {fps_no_gpu:.2f}', (20, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)

        # Measure FPS with GPU acceleration
        start_time = time.time()

        # Detect objects in the frame (with GPU acceleration)
        detect_objects(frame, net, classes)

        end_time = time.time()
        fps_with_gpu = 1 / (end_time - start_time)

        # Display FPS (with GPU acceleration)
        cv2.putText(frame, f'FPS (With GPU): {fps_with_gpu:.2f}', (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
